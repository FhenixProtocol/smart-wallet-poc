 ...
 */
function encrypt<T extends any[]>(item: [...T], fhenixClient: FhenixClientSync): [...EncryptableToEncrypted<T>];
function encrypt<T>(item: T, fhenixClient: FhenixClientSync): EncryptableToEncrypted<T>;
function encrypt<T>(item: T, fhenixClient: FhenixClientSync) {
  // Check if the item is a EncryptableBase and return 'encrypted'
  if (isEncryptablePrimitive(item)) {
    return item.encrypt(fhenixClient);
  }

  // If the item is an array, recursively process each element
  if (Array.isArray(item)) {
    return item.map(nestedItem => encrypt(nestedItem, fhenixClient));
  }
